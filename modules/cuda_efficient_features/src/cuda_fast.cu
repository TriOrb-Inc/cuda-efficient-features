...
void calcKeypoints(const GpuMat& image, const GpuMat& mask, GpuMat& keypoints, int nfeatures, int threshold,
	GpuMat& d_buffer, HostMem& h_buffer, cudaStream_t stream, const char* sensorId, const std::uint64_t timestamp,
	const int slotIndex, const int level)
{
	...
	logCalcKeypointsFingerprint(
		image, mask, keypoints, *h_count, keypoints.cols, nfeatures, threshold, block, grid, sensorId, timestamp,
		slotIndex, level);
}
